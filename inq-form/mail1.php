...
function value_check($POST){
	...
	if(isset($_POST["require2"])){
		$reqnames = explode(",",$POST["require2"]);
		foreach($reqnames as $reqname){
			// delete by 2015.09.09 ||$SENDS[$reqname]["error"]==4
			// ファイル添付を許可していないから必要なし
			// あるサーバーでのバージョンアップ時のWarring対応（php version 4.3.11→5.5.28）
			if(!array_key_exists($reqname, $SENDS)){
			//if(!array_key_exists($reqname, $SENDS)||$SENDS[$reqname]["error"]==4){
				 $_SESSION[$reqname]=$formitem[$reqname]=mb_convert_encoding('<p class="error">選択しておりません。この項目は必須です。</p>',"EUC-JP");
				 $error++;
			}
		}
	}
	...
	foreach($POST as $key => $value){
		...
		for($i=0; $i<count($name); $i++){
			...
			//職業
			}elseif( stristr($name[0],"Work")!==false ){
				$value=mb_convert_kana($value, "RNASKH");
				if($name[0] == "Work" && mb_strlen($value) < 1 && NAMECHECK == true){
			...
			//郵便番号
				...
				if(!preg_match("/(^\d{3}\-\d{4}$)|(^\d{7}$)/", $value)) {
					...
					$formitem[$name[0]] .= mb_convert_encoding($value, "EUC-JP");
					$formitem[$name[0]] .= "</p><br /><p class=\"error\">郵便番号の書式に誤りがあります。</strong>";
				}

			//都道府県
			}elseif( stristr($name[0],"pref")!==false ){
				$value=mb_convert_kana($value, "RNASKH");
				if($name[0] == "pref" && mb_strlen($value) < 1 && NAMECHECK == true){
					$formitem[$name[0]]=mb_convert_encoding('<p class="error">選択されていません。この項目は必須入力です。</p>', "EUC-JP");
					$error++;
				}else{
					$formitem[$name[0]] = preFilter($value);
					$formitem[$name[0]] = mb_convert_encoding($value, "EUC-JP");
			...
			//市区町村
			}elseif( stristr($name[0],"address")!==false ){
				$value=mb_convert_kana($value, "RNASKH");
				if($name[0] == "address" && mb_strlen($value) < 1 && NAMECHECK == true){
					$formitem[$name[0]]=mb_convert_encoding('<p class="error">入力されていません。この項目は必須です。</p>', "EUC-JP");
					$error++;
				}else{
					$formitem[$name[0]]=htmlentities($value,ENT_QUOTES, TEXTCODE);
			...
			//FAX
			}elseif( stristr($name[0],"fax")!==false && mb_strlen($value) > 0){
				if(strpos($value,"-")===false){
					if (!preg_match("/(^(?<!090|080|070)\d{10}$)|(^(090|080|070)\d{8}$)|(^0120\d{6}$)|(^0080\d{7}$)/", $value)) {
						$formitem[$name[0]]="<p class=\"error\">";
						// 2011.03.08エスケープ処理対応
						$formitem[$name[0]] = preFilter($value);
						$formitem[$name[0]].="</p><br /><p class=\"error\">FAXの書式に誤りがあります。</p>";
						$error++;
					}else{
						$formitem[$name[0]]=$value;
											}
...
{
	global $inputs, $endMassage,$replyfoot,$replymessage,$replycomment;
